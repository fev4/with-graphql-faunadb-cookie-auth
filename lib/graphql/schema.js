...
const contextlink = setContext((_, previousContext) => {
  ...
  if (req.headers.cookie) {
    ...
    if (customCookie) {
      console.log(
        '   query -- setContext -- Found custom cookie. Re-setting headers with it.\n'
      );
      token = customCookie;
    }
  }
